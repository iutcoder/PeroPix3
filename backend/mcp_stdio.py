...
def _req(path: str, body: dict | None = None) -> dict:
    ...
    #   ★앱이 띄운 CLI 엔진은 **안**이다 (`cliagent.mcp_spec` 이 `PEROPIX_INSIDE=1` 을 준다 — 사용자 결정
    #     2026-09-07, 1안). 같은 다리를 쓰면서 바깥으로 밝히면 대화 도구가 거절되고 승인 카드가 안 뜬다.
    head = {} if os.environ.get("PEROPIX_INSIDE") == "1" else {"X-PeroPix-Outside": "1"}
    if data:
