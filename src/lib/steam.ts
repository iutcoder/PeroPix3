 ...
 *  실측 2026-09-04 — **이 함수로** 잰 값이다 (다섯 비율 × 여덟 씨앗의 **최저**,
 *  가장 나쁜 조건인 「부드럽게 0」. 잰 스크립트는 남기지 않았고, 다시 재려면 이 상수만 바꿔
 *  `steam.test.ts` 의 덮임 판정을 돌리면 같은 수가 나온다):
 *
 *      배율    완전 덮임(알파 255)   거의 덮임(알파 ≥250)   그리는 크기
 *      ×1.5          60%                  67%             박스의 2.55배
 *      ×1.8          79%                  84%                   3.06배
 *      ×2.0          89%                  93%                   3.40배
 *      ×2.05         91%                  95%                   3.48배
 *      ×2.1          93%                  96%                   3.57배   ← 지금 값
 *      ×2.3          97%                  98%                   3.91배
 *
